...
int PiskvorkyAI::minimax(const Board& board, char player, int depth, int *move_r, int *move_c, int alpha, int beta, bool prune)
{
...
vector<float> PiskvorkyAI::maxn(const Board& board, char player, char players, int depth, int *move_r, int *move_c)
{
    if (depth == 0 || board.is_game_over())
    {
        debug_counter += 1;
